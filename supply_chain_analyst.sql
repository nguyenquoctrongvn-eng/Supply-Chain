...
WITH Top10Revenue AS (
    SELECT TOP 10
        p.Product_Name,
        SUM(od.Sales) AS Product_Revenue
    FROM [product] p
    JOIN [order details] od
...
SELECT
    SUM(t.Product_Revenue) AS Top10_Revenue,
    tr.Total_Revenue,
    (SUM(t.Product_Revenue) * 100.0 / tr.Total_Revenue) AS Contribution_Percent
FROM Top10Revenue t
...
GROUP BY p.Product_Name
HAVING (SUM(od.Order_Item_Profit_Ratio * od.Sales) / SUM(od.Sales)) * 100 < 10 
...
-- Delivery Delay Rate Across Customer Segments
SELECT
    c.Customer_Segment,
    COUNT(DISTINCT o.Order_Id) AS Total_Orders,
    COUNT(DISTINCT CASE
        WHEN o.Delivery_Status = 'Late delivery'
        THEN o.Order_Id
    END) * 100.0
    / COUNT(DISTINCT o.Order_Id) AS Late_Delivery_Rate
FROM [order] o
JOIN customer c
ON o.Customer_Id = c.Customer_Id
GROUP BY c.Customer_Segment
...
SELECT
    ...
    SUM(CASE WHEN o.Delivery_Status = 'Late delivery' THEN 1 ELSE 0 END) AS Late_Orders,
    (SUM(CASE WHEN o.Delivery_Status = 'Late delivery' THEN 1 ELSE 0 END) * 100.0 / COUNT(*)) AS Late_Delivery_Rate
FROM [order] o
...
-- Top 10 Lowest-Profit Products Under High Discounts
SELECT TOP 10
    p.Product_Name,
    COUNT(od.Order_Id) AS Total_Orders_Discounted,
    AVG(od.Sales) AS Avg_Sales,
    AVG(od.Order_Item_Profit_Ratio * od.Sales) AS Avg_Profit
FROM [order details] od
JOIN [product] p ON od.Product_Card_Id = p.Product_Card_Id
WHERE od.Order_Item_Discount > 0.3 
GROUP BY p.Product_Name
